{-# LANGUAGE ViewPatterns, GeneralizedNewtypeDeriving, FlexibleInstances, FlexibleContexts, UndecidableInstances, IncoherentInstances, TemplateHaskell, QuasiQuotes #-}
module Interpreter
   ( resolve, resolve_
...
builtins :: [Clause]
builtins =
   [ Clause (Struct "="   [var "X", var "X"]) []                                 -- Equality of two variables, with same names is True
   , Clause (Struct "\\=" [var "X", var "X"]) [cut, Struct "false" []]           -- Inequality of two variables, with same names is False
   , Clause (Struct "\\=" [var "X", var "Y"]) []                                 -- Inequality of two variables, with different names is True
   , Clause (Struct "not" [var "A"]) [var "A", cut, Struct "false" []]           -- Negation, is False
   , Clause (Struct "not" [var "A"]) []                                          -- Negation, is True
 ...
 where
   binaryIntegerPredicate :: (Double -> Double -> Bool) -> ([Term] -> [Goal])
   ...
   eval (Struct (reads->[(n,"")]) []) = return n :: Maybe Double
