...
module XML(XmlExp(..),Encoding(..),XmlDocParams(..),
       xtxt,xml,showXmlDoc,showXmlDocWithParams,
       writeXmlFile,writeXmlFileWithParams,parseXmlString,readXmlFile,
       readUnsafeXmlFile,readFileWithXmlDocs,textOfXml) where
...
xmlUnquoteSpecial special cs
  ...
  | special=="auml" = chr 228 : xmlUnquoteSpecials cs
  | special=="ouml" = chr 246 : xmlUnquoteSpecials cs
  | special=="uuml" = chr 252 : xmlUnquoteSpecials cs
  | special=="Auml" = chr 196 : xmlUnquoteSpecials cs
  | special=="Ouml" = chr 214 : xmlUnquoteSpecials cs
  | special=="Uuml" = chr 220 : xmlUnquoteSpecials cs
  | special=="szlig"= chr 223 : xmlUnquoteSpecials cs
...
---
--- For instance,
--- <code>textOfXml [XText "xy", XElem "a" [] [], XText "ab"] == "xy ab"</code>
textOfXml :: [XmlExp] -> String
textOfXml xexps =
   concat (intersperse " " (filter (not . null) (map textOfXmlItem xexps)))
 where
   textOfXmlItem (XText s) = s
   textOfXmlItem (XElem _ _ xs) = textOfXml xs
